...
static void	message_store(char **msg, size_t *msg_len, unsigned char c)
{
	char	*tmp;
...
static void	bit_consume(char **msg, size_t *msg_len)
{
	...
	if (g_srv.bit_idx == 8)
	{
		...
		g_srv.bit_idx = 0;
		g_srv.full_char = 0;
		message_store(msg, msg_len, complete_char);
	}
}
